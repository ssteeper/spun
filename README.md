...
The rAF loop runs only while something grows, settles, condenses, fades or zooms, or while the Sunlit breeze is above 0. `web/sw.js` precaches the shell, `index.json` and the default specimen. `.silk` files are cache-first and `index.json` is stale-while-revalidate. Navigations and shell files are network-first, so online every file comes from one release; offline they come from the cache, and navigations fall back to `offline.html`. A page loaded while an older, cache-first worker still controls it can get the previous release's files, so an inline script in `index.html` reloads it once when the new worker takes over, unless the current `app.js` is already running. The cache version is a content hash written by `tools/build.py`. `?look=classic|sunlit` selects a look. `?debug=1` exposes `window.__spun`:
